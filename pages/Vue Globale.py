...
def construire_historique_couverture(
    ...
    """
    Reconstruit, à la fin de chaque mois, le nombre et le taux d'ESI
    couverts par au moins un contrat pour toutes les entités de la maille.

    La date de début contractuelle est utilisée en priorité. Si elle est
    absente, la date de création dans Intent sert de date de repli.

    Faute d'historique de création/désactivation des ESI, le dénominateur
    correspond au parc ESI actuellement présent dans le périmètre filtré.
    """
    ...
    if (
        esi.empty
        or contrats.empty
        or "esi_reference" not in esi.columns
        or "esi_reference" not in contrats.columns
        or maille not in esi.columns
    ):
        return pd.DataFrame()

    # Nettoyage du parc ESI et conservation de toutes les entités,
    # y compris les valeurs organisationnelles non renseignées.
    esi = esi[esi["esi_reference"].notna()].copy()
    esi["esi_reference"] = normaliser_reference(esi["esi_reference"])
    esi = esi[esi["esi_reference"].notna()].copy()
    esi[maille] = nettoyer_texte(esi[maille])
    esi[maille] = esi[maille].replace("", "Non renseigné").fillna("Non renseigné")

    # Une ligne maximum par ESI dans son rattachement organisationnel actuel.
    esi = esi.sort_values([maille, "esi_reference"]).drop_duplicates("esi_reference")

    # Début réel du contrat : date de début, sinon date de création Intent.
    contrats["_date_debut_couverture"] = contrats["contract_start_date"].combine_first(
        contrats["contract_creation_date"]
    )

    # Fin réelle : première date connue entre la fin contractuelle et
    # la désactivation dans Intent.
    dates_fin = contrats[["contract_end_date", "contract_deactivation_date"]].copy()
    contrats["_date_fin_couverture"] = dates_fin.min(axis=1)

    contrats = contrats[
        contrats["esi_reference"].notna()
        & contrats["_date_debut_couverture"].notna()
    ].copy()

    if contrats.empty:
        return pd.DataFrame()

    contrats["esi_reference"] = normaliser_reference(contrats["esi_reference"])
    contrats = contrats[contrats["esi_reference"].notna()].copy()

    aujourd_hui = aujourd_hui_france()
    fin = aujourd_hui.to_period("M").to_timestamp("M")
    debut = (
        fin - pd.DateOffset(months=max(int(nb_mois) - 1, 0))
    ...
    # Toutes les entités de la maille sélectionnée sont calculées.
    for entite, groupe_esi in esi.groupby(maille, dropna=False, sort=True):
        refs_esi = set(liste_refs_valides(groupe_esi, "esi_reference"))
        total_esi = len(refs_esi)

        if total_esi == 0:
            continue

        sous_contrats = contrats[
            contrats["esi_reference"].astype(str).isin(refs_esi)
        ].copy()

        for fin_mois in mois:
            # Pour le mois en cours, la photographie s'arrête à aujourd'hui
            # afin de ne pas compter un éventuel contrat qui commencerait plus tard.
            date_observation = min(fin_mois, aujourd_hui)
            actifs_fin_mois = sous_contrats[
                (sous_contrats["_date_debut_couverture"] <= date_observation)
                & (
                    sous_contrats["_date_fin_couverture"].isna()
                    | (sous_contrats["_date_fin_couverture"] >= date_observation)
                )
            ]

            nb_couverts = int(actifs_fin_mois["esi_reference"].nunique())
            taux_couverture = round(taux(nb_couverts, total_esi), 2)

            lignes.append(
                {
                    "Mois": fin_mois,
                    "Date d’observation": date_observation,
                    "Mois affiché": f"{noms_mois[fin_mois.month]} {fin_mois.year}",
                    "Maille": maille.capitalize(),
                    "Entité": str(entite),
                    "ESI couverts": nb_couverts,
                    "ESI non couverts": int(max(total_esi - nb_couverts, 0)),
                    "Total ESI": int(total_esi),
    ...
    return (
        pd.DataFrame(lignes)
        .sort_values(["Mois", "Entité"], kind="stable")
        .reset_index(drop=True)
    )


def afficher_historique_couverture(
    df: pd.DataFrame,
    indicateur: str,
) -> None:
    """Affiche l'évolution mensuelle pour toutes les entités calculées."""
    if df.empty:
        st.info(
            "L’historique nécessite au minimum une date de début ou une date "
            "de création exploitable pour les contrats rattachés aux ESI."
        )
        return

    colonne_valeur = (
        "ESI couverts"
        if indicateur == "Nombre d’ESI couverts"
        else "Taux de couverture (%)"
    )
    ...
    fig = go.Figure()
    ordre_mois = (
        df[["Mois", "Mois affiché"]]
        .drop_duplicates()
        .sort_values("Mois")["Mois affiché"]
        .tolist()
    )

    for entite, groupe in df.groupby("Entité", sort=True):
        groupe = groupe.sort_values("Mois")
        fig.add_trace(
            go.Scatter(
                x=groupe["Mois affiché"],
                y=groupe[colonne_valeur],
                mode="lines+markers",
                name=str(entite),
                customdata=groupe[
                    ["ESI couverts", "ESI non couverts", "Total ESI", "Taux de couverture (%)"]
                ],
                hovertemplate=(
                    f"<b>{safe(entite)}</b><br>"
                    "%{x}<br>"
                    "ESI couverts : %{customdata[0]}<br>"
                    "ESI non couverts : %{customdata[1]}<br>"
                    "Total ESI : %{customdata[2]}<br>"
                    "Taux : %{customdata[3]:.1f} %<extra></extra>"
                ),
            )
        )

    nb_entites = int(df["Entité"].nunique())
    hauteur = max(470, min(820, 430 + nb_entites * 8))
    layout_plotly(fig, hauteur, showlegend=True)

    yaxis = dict(
        title=(
            "Nombre d’ESI couverts"
            if indicateur == "Nombre d’ESI couverts"
            else "Taux de couverture (%)"
        ),
        gridcolor=C_GRID,
    ...
    if indicateur == "Taux de couverture":
        yaxis["range"] = [0, 105]
        yaxis["ticksuffix"] = " %"
    ...
    fig.update_layout(
        ...
        yaxis=yaxis,
        hovermode="x unified" if nb_entites <= 12 else "closest",
        legend=dict(
            title="Entités",
            orientation="v",
            yanchor="top",
        ...
        margin=dict(l=18, r=230, t=42, b=90),
    )
    ...
    st.plotly_chart(
        ...
        config={
            ...
            "toImageButtonOptions": {
                "format": "png",
                "filename": "evolution_mensuelle_couverture_esi",
                "scale": 2,
            },
        },
    )
...
def afficher_barres_horizontales(
    ...
    if df.empty:
        ...
        return
    graphe = df.head(top_n).sort_values(value_col, ascending=True)
    if go is None:
    ...
    layout_plotly(fig, max(390, 31 * len(graphe) + 100))
    fig.update_layout(
        xaxis=dict(title=None, gridcolor=C_GRID),
        yaxis=dict(title=None, automargin=True),
        margin=dict(l=12, r=55, t=14, b=35),
    )
    st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
...
# =====================================================
    ...
    section(
        ...
        (
            "Évolution, à la fin de chaque mois, du nombre et du taux d’ESI "
            "couverts par au moins un contrat. Toutes les entités de la maille "
            "sélectionnée sont incluses dans le calcul et dans l’export."
        ),
    )

    hist_col1, hist_col2, hist_col3 = st.columns([1, 1, 1.35])
    ...
    with hist_col3:
        indicateur_historique = st.radio(
            "Indicateur affiché",
            ["Taux de couverture", "Nombre d’ESI couverts"],
            horizontal=True,
            key="coverage_historique_indicateur",
        )
    ...
    historique_couverture = construire_historique_couverture(
        df_esi=df_esi_contexte,
        df_contrats=df_contrats_recherche,
        maille=maille_historique_colonne,
        nb_mois=int(periode_couverture),
    )

    if not historique_couverture.empty:
        nb_entites_hist = int(historique_couverture["Entité"].nunique())
        nb_mois_hist = int(historique_couverture["Mois"].nunique())
        derniere_date_hist = historique_couverture["Mois"].max()
        derniere_photo = historique_couverture[
            historique_couverture["Mois"] == derniere_date_hist
        ]
        total_esi_hist = int(derniere_photo["Total ESI"].sum())
        esi_couverts_hist = int(derniere_photo["ESI couverts"].sum())
        taux_global_hist = taux(esi_couverts_hist, total_esi_hist)
        ...
        with r1:
            alert_card(
                "Entités comparées",
                nb_entites_hist,
                f"Toutes les entités de la maille {maille_historique_label.lower()}.",
                C_BLUE,
            )
        with r2:
            alert_card(
                "Mois disponibles",
                nb_mois_hist,
                "Les mois sans nouveau contrat restent affichés.",
                C_BLUE_LIGHT,
            )
        with r3:
            alert_card(
                "ESI couverts au dernier mois",
                esi_couverts_hist,
                f"Sur {fmt_nombre(total_esi_hist)} ESI dans le périmètre.",
                C_TEAL,
            )
        with r4:
            alert_card(
                "Taux au dernier mois (%)",
                taux_global_hist,
                "Photographie à la fin du mois le plus récent.",
                C_RED,
                decimales=1,
            )

        st.caption(
            "Le parc ESI historique n’étant pas daté dans la source actuelle, "
            "le dénominateur utilise les ESI actuellement présents dans le "
            "périmètre filtré. La couverture est mesurée à la fin de chaque mois."
        )

    afficher_historique_couverture(
        historique_couverture,
        indicateur_historique,
    )

    if not historique_couverture.empty:
        with st.expander("Consulter toutes les données mensuelles", expanded=False):
            table_historique = historique_couverture[
                [
                    "Mois",
                    "Date d’observation",
                    "Mois affiché",
            ...
            st.dataframe(
                formatter_dates_table(table_historique),
                width="stretch",
                hide_index=True,
                height=460,
            )
        ...
        dataframe_download(
            "Télécharger toutes les données de l’évolution",
