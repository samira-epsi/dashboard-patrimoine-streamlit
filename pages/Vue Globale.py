...
def dataframe_download(label: str, df: pd.DataFrame, filename: str) -> None:
    """Télécharge un DataFrame au format Excel avec filtres et volets figés."""
    if df.empty:
        return

    export = df.copy()
    for colonne in export.columns:
        if pd.api.types.is_datetime64_any_dtype(export[colonne]):
            serie = pd.to_datetime(export[colonne], errors="coerce")
    ...
    with pd.ExcelWriter(fichier, engine="openpyxl") as writer:
        ...
        feuille = writer.book["Données"]
        feuille.freeze_panes = "A2"
        feuille.auto_filter.ref = feuille.dimensions

        for cellules in feuille.columns:
            valeurs = [str(cellule.value) if cellule.value is not None else "" for cellule in cellules]
            largeur = min(max((len(valeur) for valeur in valeurs), default=0) + 3, 50)
    ...
    st.download_button(
        ...
        width="stretch",
        key=f"excel_{nom_excel}_{abs(hash(tuple(export.columns)))}",
        on_click="ignore",
    )
...
def afficher_table_paginated(
    ...
    dataframe_download("Télécharger toutes les lignes filtrées en Excel", df, filename)
...
def config_graphique_exportable(nom_fichier: str) -> dict:
    """Configuration Plotly sans barre d’outils anglaise."""
    return {
...
def afficher_graphique_exportable(fig, nom_fichier: str) -> None:
    """Affiche le graphique et un bouton français de téléchargement PNG."""
    st.plotly_chart(
        fig,
        use_container_width=True,
        config=config_graphique_exportable(nom_fichier),
    )
    try:
        image_png = fig.to_image(
            format="png",
            width=1600,
            height=900,
            scale=2,
        ...
        st.download_button(
            ...
            mime="image/png",
            key=f"png_{nom_fichier}",
            width="stretch",
            on_click="ignore",
        )
    except Exception:
        st.caption(
            "Export PNG indisponible : ajoute kaleido dans requirements.txt."
        )
...
def configurer_axe_temporel(fig, periodes: pd.Series, granularite: str) -> None:
    """Crée un axe lisible, en français, même dans une demi-colonne."""
    dates = pd.to_datetime(periodes, errors="coerce").dropna().drop_duplicates().sort_values()
    if dates.empty:
        return

    if granularite == "Annuel":
        tickvals = dates.tolist()
        ticktext = [str(date.year) for date in dates]
        angle = 0
    else:
        # En demi-largeur, on limite à environ 6 libellés visibles.
        pas = max(1, math.ceil(len(dates) / 6))
        selection = dates.iloc[::pas]
        if selection.iloc[-1] != dates.iloc[-1]:
            selection = pd.concat([selection, dates.iloc[[-1]]]).drop_duplicates()
        tickvals = selection.tolist()
        ticktext = [f"{MOIS_FR_COURTS[date.month - 1]} {date.year}" for date in selection]
        angle = -35
...
def afficher_comparaison_organisation(df: pd.DataFrame, indicateur: str) -> None:
    ...
    afficher_graphique_exportable(fig, "comparaison_organisationnelle")
...
def afficher_couverture_metier(df: pd.DataFrame) -> None:
    ...
    afficher_graphique_exportable(fig, "couverture_par_metier")
...
def afficher_historique_couverture(df: pd.DataFrame) -> None:
    ...
    afficher_graphique_exportable(fig, "evolution_couverture")
...
def afficher_cycle_flux(df: pd.DataFrame, granularite: str) -> None:
    ...
    fig.update_layout(
        ...
        margin=dict(l=12, r=22, t=44, b=82),
    )
    configurer_axe_temporel(fig, df["Période"], granularite)
    afficher_graphique_exportable(fig, "evolution_contrats_crees_desactives")
...
def afficher_cycle_stock(df: pd.DataFrame, granularite: str) -> None:
    ...
    configurer_axe_temporel(fig, df["Période"], granularite)
    afficher_graphique_exportable(fig, "evolution_stock_contrats")
...
def afficher_donut_statuts(df_master: pd.DataFrame) -> None:
    ...
    afficher_graphique_exportable(fig, "repartition_statut_contrats")
...
def afficher_barres_horizontales(
    ...
    afficher_graphique_exportable(fig, nom_export)
...
if vue_active == "Vue globale":
    ...
    with st.expander("Consulter la liste des contrats", expanded=False):
        ...
        if mode_tableau == "Contrats uniques":
            ...
            nom_export = "contrats_uniques.xlsx"
            ...
            nom_export = "contrats_rattachements.xlsx"
            ...
            nom_export = "contrats_codes_prestation.xlsx"
    ...
    if not historique_cycle.empty:
        dataframe_download(
            "Télécharger les données d’évolution des contrats en Excel",
            formatter_dates_table(historique_cycle),
            "evolution_contrats_intent.xlsx",
        )
...
# =====================================================
    ...
    if not comparaison_org.empty:
        dataframe_download(
            ...
            comparaison_org,
            "comparaison_couverture_organisation.xlsx",
        )
    ...
    if not couverture_metier.empty:
        dataframe_download(
            ...
            couverture_metier,
            "couverture_esi_par_metier.xlsx",
        )
    ...
    if not historique_couverture.empty:
        dataframe_download(
            ...
            "evolution_mensuelle_couverture_esi.xlsx",
        )
    ...
    nom_fichier_liste = "liste_couverture.xlsx"
    if population == "Tous les ESI":
        ...
        if df_equipements_filtre.empty:
            ...
            table_liste = pd.DataFrame()
        nom_fichier_liste = "equipements_sans_couverture_valide.xlsx"
    else:
        if df_equipements_filtre.empty:
            table_liste = pd.DataFrame()
        elif "equipment_covered_valid" in df_equipements_filtre.columns:
            ...
            table_liste = pd.DataFrame()
        nom_fichier_liste = "equipements_couverts_valides.xlsx"
    ...
    if sous_vue == "Anomalies de données":
        ...
        if qualite.empty:
            ...
            afficher_table_paginated(
                ...
                filename="anomalies_donnees.xlsx",
                hauteur=500,
            )
        ...
        if alertes.empty:
            ...
            afficher_table_paginated(
                table_alertes,
                key="quality_alertes",
                filename="alertes_couverture.xlsx",
                hauteur=500,
            )
